...
public class ClientDao implements ClientInterface {
    ...
    public List<Client> Search(String attribute) {
        ...
        try {
            PreparedStatement statement = connection.prepareStatement("SELECT * FROM person AS pr INNER JOIN client as cl ON cl.id = pr.id  where code Like ? OR firstname Like ? OR lastName Like ? OR phonenumber Like ? OR adress Like ?;");
            ...
            statement.setString(5,"%"+attribute+"%");
            ResultSet resultSet = statement.executeQuery();
